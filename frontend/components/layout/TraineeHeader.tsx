...
import { ShieldCheck, Bell, Menu, ChevronDown } from 'lucide-react';

export const TraineeHeader: React.FC = () => {
  const { user } = useAuth();
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 font-medium">State Livelihood Outcome Mission • Trainee Portal</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {user && (
              <div className="hidden sm:flex items-center gap-2 pl-2 border-l border-slate-200">
                <div className="w-8 h-8 rounded-full bg-slate-900 text-white font-bold text-xs flex items-center justify-center">TR</div>
                <div className="text-xs">
                  <div className="font-extrabold text-slate-900 flex items-center gap-1">
                    {user.name.split(' ')[0]} <ChevronDown className="w-3 h-3 text-slate-400" />
                  </div>
                  <div className="text-[10px] text-slate-500 font-mono">Trainee</div>
                </div>
              </div>
            )}
