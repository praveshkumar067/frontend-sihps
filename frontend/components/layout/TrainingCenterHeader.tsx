...
import { ShieldCheck, Bell, Menu, ChevronDown, Send } from 'lucide-react';
import Link from 'next/link';

export const TrainingCenterHeader: React.FC = () => {
  const { user } = useAuth();
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 font-medium">State Livelihood Outcome Mission • Training Center Portal</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors"
            >
            ...
            {user && (
              <div className="hidden sm:flex items-center gap-2 pl-2 border-l border-slate-200">
                <div className="w-8 h-8 rounded-full bg-slate-900 text-white font-bold text-xs flex items-center justify-center">TC</div>
                <div className="text-xs">
                  <div className="font-extrabold text-slate-900 flex items-center gap-1">
                    {user.name.split(' ')[0]} <ChevronDown className="w-3 h-3 text-slate-400" />
                  </div>
                  <div className="text-[10px] text-slate-500 font-mono">Training Center</div>
                </div>
              </div>
            )}
