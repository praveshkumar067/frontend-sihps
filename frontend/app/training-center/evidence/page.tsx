...
export default function TrainingCenterEvidencePage() {
  const { user } = useAuth();
  ...
  const fetchEvidence = async () => {
    if (!tc) return;
    try {
      const data = await api.getEvidenceList(tc.training_center_id);
      setSubmissions(data);
    } catch (err) {
      console.error(err);
    }
  ...
  const currentTier = tc ? tc.trust_tier : 3;
  const verificationStatus = tc ? tc.verification_status : 'verified';
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800">
        <div>
          <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-md mb-2">
            <FileCheck className="w-3.5 h-3.5" /> TrustLevel Module (Training Center Role)
          </div>
